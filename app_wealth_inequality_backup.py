...
app.layout = html.Div([
    ...
    dcc.Interval(id='interval-component', interval=50, n_intervals=0),  # Always running, check state in callback
...
def control_simulation(start_clicks, stop_clicks, reset_clicks,
    ...
    if not ctx.triggered:
        if greedy_ratio + neutral_ratio > 1.0:
            return (None, False, "Error: Greedy + Neutral ratios cannot exceed 1.0!", 
        ...
        return (sim_data, False, "", False, True, start_enabled_style, stop_disabled_style)
    
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    
    if button_id == 'start-btn':
        if greedy_ratio + neutral_ratio > 1.0:
            return (sim_data, False, "Error: Greedy + Neutral ratios cannot exceed 1.0!", 
                    False, True, start_enabled_style, stop_disabled_style)
        ...
        return (sim_data, True, "", True, False, start_disabled_style, stop_enabled_style)
...
# Main update callback - runs on interval tick only
@app.callback(
...
def update_simulation(n_intervals, sim_data, is_running, speed_multiplier):
    """Update simulation and display - triggered by interval ticks only"""
    
    try:
        if sim_data is None:
            return create_empty_outputs()[:-1] + (sim_data,)
        ...
        if is_running:
            ...
            max_steps = min(int(speed_multiplier), 1000)
...
# Initial display callback - shows state immediately after Start/Reset
@app.callback(
    [Output('status-bar', 'children', allow_duplicate=True),
     Output('key-metrics', 'children', allow_duplicate=True),
     Output('inequality-chart', 'figure', allow_duplicate=True),
     Output('wealth-hist', 'figure', allow_duplicate=True),
     Output('survival-chart', 'figure', allow_duplicate=True),
     Output('concentration-chart', 'figure', allow_duplicate=True),
     Output('results-table', 'children', allow_duplicate=True)],
    [Input('sim-state', 'data')],
    [State('running-state', 'data')],
    prevent_initial_call=True
)
def update_display_on_state_change(sim_data, is_running):
    """Update display immediately when sim-state changes (Start/Reset button)"""
    try:
        if sim_data is None:
            return create_empty_outputs()[:-1]
        ...
        print(f"Error in update_display_on_state_change: {e}")
        import traceback
        traceback.print_exc()
        return create_empty_outputs()[:-1]
